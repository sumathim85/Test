...
from .console.prodapt_update_db import UpdateDB
...
class Logger(AbstractLogger):
    ...
    def __init__(self, register_console_logger=True):
        ...
        self._db= UpdateDB()
        self._db.set_Id()
        self._test,self._extra=self._db.log_ur_data()
    ...
    def _log_message(self, msg, no_cache=False):
        ...
        if msg.level in ('WARN', 'ERROR'):
            self.message(msg)
        if msg.level in ('WARN', 'INFO', 'FAIL','ERROR'):
            self._extra["test_case_name"]=self._testName
            self._extra["suite_name"] = self._suiteName
            self._extra["log_level"] = msg.level
            self._extra["keyword"] = self._keywordName
            self._test(msg, extra=self._extra)
        if msg.level in ('FAIL', 'ERROR'):
            self._db.send_mail(send_to='sathishkumar.sv@prodapt.com',testase=self._testName,suite=self._suiteName,message=msg,status=msg.level)
    ...
    def custom_print(self,msg):
        self._extra["test_case_name"] = self._testName
        self._extra["suite_name"] = self._suiteName
        self._extra["log_level"] = 'INFO'
        self._extra["keyword"] = "'BuiltIn.Log'"
        self._test(msg, extra=self._extra)
