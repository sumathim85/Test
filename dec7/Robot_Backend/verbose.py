...
from .prodapt_update_db import UpdateDB
...
class VerboseOutput(object):
    ...
    def end_suite(self, suite):
        ...
        self._test(suite.longname+"         "+suite.status+"\n"+suite.full_message, extra=self._extra)
        self._writer.suite_separator()
        self._db.update_db(suite, suite.status,"suite")

        
    def start_test(self, test):
        self._writer.info(test.name, test.doc)
        self._running_test = True
        self._extra["test_case_name"]=test.name
        self._test(test.name,extra=self._extra)
        self._db.update_db(test, "Inprogress", "testcase")
        

    def end_test(self, test):
        self._writer.status(test.status, clear=True)
        self._db.update_db(test, test.status, "testcase")
        self._writer.message(test.message)
        self._writer.test_separator()
        self._running_test = False
        self._test(test.name + "         " + test.status + "\n" + test.message, extra=self._extra)
        self._test(test.status, extra=self._extra)
        del self._extra["test_case_name"]
    ...
    def output_file(self, name, path):
        ...
        self._test(name+":"+path, extra=self._extra)
