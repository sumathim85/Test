                ...
                if "." not in robot.longname or "&" in robot.longname:
                    if self._suborder_id == "0":
                        command1 = ("UPDATE order_details "
                                    "SET total = %(Total)s, pass = %(Pass)s, fail = %(Fail)s,  order_status = %(Order_Status)s "
                                    "where order_id = %(Id)s AND sub_order_id = 0")
                    else:
