    ...
    def updateelasticDG(self,robot ,savings):
        try:
            ...
            for idea in response['hits']['hits']:
                ...
                idea['_source']['runningMonth'] = now.strftime("%m(%b)-%Y")
...
#                     utc_end_time = end_time.astimezone(pytz.utc)
                ...
                no_of_testcase = [int(s) for s in line_split[0].split() if s.isdigit()]
    ...
    def failDbUpdate(self,message):
        ...
        for row in myresult:
            total = row[0]
            # print(row[0])
        cursor.execute("SELECT COUNT(bot_id)  FROM bot_details WHERE order_id='" + self._order_id + "' AND status ='PASS'")
        myresult = cursor.fetchall()
        for row in myresult:
            passCount = row[0]
            # print(row[0])
