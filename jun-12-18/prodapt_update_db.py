...
class UpdateDB(object):
    ...
    def get_role(self,id):
        try:
            ...
            if self._env.lower() == 'dg':
                cmd="SELECT role_id FROM order_details_v2 WHERE order_id='" + id + "'"
            else:
                cmd="SELECT role_id FROM order_details WHERE order_id='" + id + "'"
            cursor.execute(cmd)
    ...
    def intake_id(self, robot):
        try:
            ...
            cursor = connection.cursor()
            if self._env.lower() == 'dg':
                cmd="SELECT intake_id,suite_id FROM suite_catalogue_v2 WHERE suite_name='" + robot.name + "'"
            else:
                cmd="SELECT intake_id,suite_id FROM suite_catalogue WHERE suite_name='" + robot.name + "'"
            cursor.execute(cmd)
            myresult = cursor.fetchall()
            for row in myresult:
    ...
    def updateelasticDG(self,robot ,savings):
        try:
                ...
                "query": {
                    "bool": {
                        "filter": {
                            "script": {
                                "script": {
                                    "source": "doc['intakeId'].value == "+ str(self._intake_id) +" && doc['docType.keyword'].value == 'Idea' ",
                                    "lang": "painless"
                                }
                            }
                        }
            ...
            response = es.search(index="intake", body=search_prams)
            for idea in response['hits']['hits']:
                now = datetime.utcnow()
                idea['_source']['docType'] = 'run'
                idea['_source']['modifiedDate'] = now
                idea['_source']['runningMonth'] = now
                idea['_source']['actualRuns'] =1
                es.index(index="intake", body=idea["_source"])
...
#                     utc_end_time = end_time.astimezone(pytz.utc)
                    ...
                    if (Status == "PASS"):
                        if self._env.lower() == 'dg':
                            cmd="SELECT no_runs,savings_per_run FROM bot_catalogue_v2 WHERE bot_name='" + robot.name + "' AND suite_id='" + str(
                                self._suite_id) + "'"
                        else:
                            cmd="SELECT no_runs,savings_per_run FROM bot_catalogue WHERE bot_name='" + robot.name + "' AND suite_id='" + str(
                                self._suite_id) + "'"
                        cursor.execute(cmd)
                        myresult = cursor.fetchall()
                        # runs = 0
                        ...
                        if savingsPerRun == 0:
                            ...
                            if self._env.lower() =='dg':
                                self.updateelasticDG(robot, savingsPerRun)
                            else:
                ...
                values_to_insert = {"Elapsed": elapsed_time, "Total": no_of_testcase[0], "Pass": no_of_testcase[1],
                                    ...
                                    "Id": self._order_id, "suborderid": self._suborder_id}
                if self._env.lower() == 'dg':
                    command = ("UPDATE suite_details_v2 "
                ...
                if "." not in robot.longname or "&" in robot.longname:
                    if self._suborder_id == "0":
                        if self._env.lower() == 'dg':
                            command1 = ("UPDATE order_details_v2 "
                        ...
                        if self._env.lower() == 'dg':
                            command1 = ("UPDATE sub_order_details_v2 "
                                        "SET total = %(Total)s, pass = %(Pass)s, fail = %(Fail)s,  order_status = %(Order_Status)s, elapsed = %(Elapsed)s  "
                                        "where order_id = %(Id)s AND sub_order_id = %(suborderid)s")
                        else:
            ...
            print(robot.name)
            self.failDbUpdate("Un Expected Exception occurred at Robot Back end ,Due to test case : "+robot.name)
            exit(-1)
    ...
    def failDbUpdate(self,message):
        ...
        if self._env.lower() == 'dg':
            cmd="SELECT order_created_date FROM order_details_v2 WHERE order_id='" + self._order_id + "'"
        else:
            cmd="SELECT order_created_date FROM order_details WHERE order_id='" + self._order_id + "'"
        cursor.execute(cmd)
        myresult = cursor.fetchall()
        for row in myresult:
            start_time = row[0]
        ...
        values_to_insert = {"Status": "FAIL", "Message": message,
                            "Id": self._order_id,
                            "suborderid": self._suborder_id, "Order_Status": "FAIL","elapsed":elapsed ,"endTime":end_time_after ,"status":"PASS"}
        if self._env.lower() == 'dg':
            command = ("UPDATE bot_details_v2 "
                       "SET status = %(Status)s, message = %(Message)s ,elapsed= %(elapsed)s ,end_time=%(endTime)s "
                       "where order_id = %(Id)s AND sub_order_id = %(suborderid)s AND status != %(status)s")
        else:
            command = ("UPDATE bot_details "
                       "SET status = %(Status)s, message = %(Message)s ,elapsed= %(elapsed)s ,end_time=%(endTime)s "
                       "where order_id = %(Id)s AND sub_order_id = %(suborderid)s AND status != %(status)s")
        ...
        connection.commit()
        if self._env.lower() == 'dg':
            cmd1="SELECT COUNT(bot_id)  FROM bot_details_v2 WHERE order_id='" + self._order_id + "'"
        else:
            cmd1="SELECT COUNT(bot_id)  FROM bot_details WHERE order_id='" + self._order_id + "'"
        cursor.execute(cmd1)
        myresult = cursor.fetchall()
        for row in myresult:
            total = row[0]
            # print(row[0])
        if self._env.lower() == 'dg':
            cmd2="SELECT COUNT(bot_id)  FROM bot_details_v2 WHERE order_id='" + self._order_id + "' AND status ='PASS'";
        else:
            cmd2="SELECT COUNT(bot_id)  FROM bot_details WHERE order_id='" + self._order_id + "' AND status ='PASS'"
        cursor.execute(cmd2)
        myresult = cursor.fetchall()
        for row in myresult:
            passCount = row[0]
        ...
        values_to_insert = {"Status": "FAIL", "Message": message,
                            "Id": self._order_id,
                            "suborderid": self._suborder_id, "Total": total, "Pass": passCount, "Fail": total - passCount,
                            "Order_Status": "Failed" ,"elapsed":elapsed}
        if self._env.lower() == 'dg':
            command1 = ("UPDATE suite_details_v2 "
        ...
        connection.commit()
        if self._env.lower() == 'dg':
            command2 = ("UPDATE order_details_v2 "
